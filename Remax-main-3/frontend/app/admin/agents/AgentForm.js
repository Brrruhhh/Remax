...
export default function AgentForm({ agent, onSave, onCancel, saving }) {
  ...
  const validateForm = () => {
    ...
    // Password strength (if creating new agent)
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-h-96 overflow-y-auto pr-4">
          ...
          {states.map(state => (
            <label key={state} className="flex items-center space-x-2">
              <input
                ...
                className="rounded border-gray-300 text-remax-blue focus:ring-remax-blue"
              />
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t">
        ...
        <button
          type="submit"
          disabled={saving}
          className="px-4 py-2 bg-remax-blue text-white rounded-md hover:bg-remax-dark-blue disabled:opacity-50 flex items-center space-x-2"
        >
          {saving && (
